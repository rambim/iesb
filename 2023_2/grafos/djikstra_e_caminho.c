...
void mostrar_caminho(vertice *vertices, int inicio, int destino);
...
void djikstra(vertice * vertices, int raiz) {
    ...
    vertices[raiz].predecessor = -1; // Inicializando o predecessor do vértice raiz
    ...
    while (!empty()) {
        ...
        while (aux != NULL) {
            if (distancia_atual + aux->peso < vertices[aux->valor].distancia) {
                ...
                vertices[aux->valor].predecessor = corrente; // Definindo o predecessor
...
void mostrar_caminho(vertice *vertices, int inicio, int destino) {
    int atual = destino;
    int caminho[10001]; // Assumindo que o número de vértices não ultrapassa 10001
    int indice = 0;

    while(atual != inicio) {
        caminho[indice++] = atual;
        atual = vertices[atual].predecessor;
    }
    caminho[indice] = inicio;

    printf("Caminho de %d até %d: ", inicio, destino);
