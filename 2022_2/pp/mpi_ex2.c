...
int main()
{
    ...
    case 1:
        while (1)
        {
            ...
            printf("\n Sou a thread %d %d Aguardando alguem terminar serviço", id_processo,cc1);
            MPI_Recv(&x,1,MPI_INT,MPI_ANY_SOURCE,0,MPI_COMM_WORLD,&status);
            printf("\n Sou a thread %d %d Recebido trabalho finalizado do %d", id_processo,cc1,status.MPI_SOURCE);
            printf("\n Sou a thread %d %d Gravando em arquivo", id_processo,cc1);
        }
        break;
    case 2:
        while (1)
    ...
    case 3:
        ...
        {
            start = MPI_Wtime();
            while (MPI_Wtime() - start < 30)
            {
                // We keep looping until <waiting_time> seconds have elapsed
            }
            cc2++;
            printf("\n Sou a thread %d %d", id_processo,cc3);
            printf("\n Sou a thread %d %d Avisando que estou atoa", id_processo,cc3);
            MPI_Send(&x,1,MPI_INT,0,0,MPI_COMM_WORLD);
            printf("\n Sou a thread %d %d Enviado!", id_processo,cc3);
            printf("\n Sou a thread %d %d Esperando servico chegar!", id_processo,cc3);
            MPI_Recv(&x,1,MPI_INT,0,0,MPI_COMM_WORLD,&status);
            printf("\n Sou a thread %d %d Trabalho chegou", id_processo,cc3);
            printf("\n Sou a thread %d %d Trabalhando..", id_processo,cc3);
            printf("\n Sou a thread %d %d Finalizado. Devolvendo resultado", id_processo,cc3);
            MPI_Send(&x,1,MPI_INT,1,0,MPI_COMM_WORLD);
        }
        break;
    default:
